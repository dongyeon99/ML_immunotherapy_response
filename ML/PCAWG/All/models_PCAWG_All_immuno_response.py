...
def RFC(x_train, x_test, y_train, y_test, model_save_folder):
    ...
    CTL_balanced_AUC_test = balanced_accuracy_score(y_test, CTL_best_model_pred)
    
    print("## CTL Classification Prediction ##")
    print("CTL_F1_Score:", CTL_F1_test)
    print("CTL_Accuracy:", CTL_AUC_test)
    print("CTL_Balanced_Accuracy:", CTL_balanced_AUC_test)
    print("-------------------------------------")
    print(" ")
    
    # Trained model save
    joblib.dump(CTL_best_model, os.path.join(model_save_folder,"PCAWG_All_RFC_CTL.pkl"))
...
def RFR(x_train, x_test, y_train, y_test, model_save_folder):
    warnings.filterwarnings("ignore")
    ...
    # best parameter
    print("Random Forest Regression best parameter ({})".format(i))
    best_parameters = grid_search.best_estimator_.get_params()
    ...
    print('best RFR MSE ({}):'.format(i), RFR_mse)
    print("-------------------------------------")
    print(" ")
 
    # Trained model save
    joblib.dump(best_RFR, os.path.join(model_save_folder,"PCAWG_All_RFR_{}.pkl".format(i)))
